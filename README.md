...
- **app-v12 fork (the cure):** post-fork, the AppHash excludes the `state:` namespace, so an idle chain's hash reaches a fixed point and empty-block production stops — height advances on real transactions (with rare bounded exceptions: epoch-boundary and governance bookkeeping writes can still move the AppHash and mint a proof block or two). Pre-fork blocks replay byte-identically; activation is governance-gated like app-v7…v11: `sage-gui upgrade propose` one fork at a time until `upgrade status` reports app-v12.
...
**Operator runbook — upgrading an existing chain:** the forks activate strictly one at a time. Run `sage-gui upgrade status` to see the chain's current app version, then `sage-gui upgrade propose --target <next>`, and repeat status → propose until `upgrade status` reports the top version this binary supports. One caveat: once the chain is at app-v8 or later, each propose must be signed by a chain-admin agent — usually your operator `agent.key` once it has performed any admin op; if a different key holds the on-chain admin role, pass it with `--agent-key <chain-admin-key>`.
