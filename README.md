...
## What's New in v7.0 (in progress)

The v7.0 cycle is focused on **recall quality** and **ambient capture**. v6.8.8 is the latest tagged release; v7.0 work is landing on `main` ahead of the tag.

- **Hybrid recall (BM25 ⊕ vector via RRF)** — `sage_recall` now fuses FTS5/BM25 and vector cosine results in one round trip via weighted Reciprocal Rank Fusion (RRF), instead of picking one index based on vault state. The fusion runs both streams in parallel, merges by memory_id with `score = bm25_weight / (k + rank_bm25) + vector_weight / (k + rank_vector)`, and returns a single ranked list. New `POST /v1/memory/hybrid` endpoint backs it, with full RBAC / multi-org / classification / decay parity vs. the existing query and search paths. Mixed-version networks keep working: older nodes that don't expose `/v1/memory/hybrid` get an automatic fall-back to the legacy FTS5 path. Defaults: `RRFK=60`, `BM25=0.4`, `Vector=0.6`, `Oversample=2`. Tune at runtime via `SAGE_HYBRID_RRF_K`, `SAGE_HYBRID_BM25_WEIGHT`, `SAGE_HYBRID_VECTOR_WEIGHT`, `SAGE_HYBRID_OVERSAMPLE`. Force the legacy single-index path with `SAGE_RECALL_HYBRID=0`.
- **Branch-aware memory tagging** — memories written from a git working tree are auto-tagged with `branch:<name>` so `feature/x` and `main` stay separable without manual hygiene. The branch is detected once per session with a 30s TTL cache and a 750ms hard timeout so a wedged `git` can't stall a memory write. Outside a git repo: silent no-op. Opt out per process: `SAGE_BRANCH_TAG=0`. The tag uses the existing tag filter — `sage_recall ... tags=["branch:feature/x"]` works the same as any other tag.
- **Claude Code lifecycle hooks** — three new hook scripts under `.claude/hooks/` close common forgetting patterns: `PreCompact` nudges `sage_reflect` before context compression discards detail, `UserPromptSubmit` reminds the agent to call `sage_turn` early in each response, and a `Stop` placeholder is in place for v7.1 end-of-turn capture. Drop the `.claude/` directory into your project to adopt them — install guide in [`docs/HOOKS.md`](docs/HOOKS.md).
